  ...
  properties: {
    ...
    publish: {
      ...
      properties: {
        ...
        history_filename: { type: 'string' },
        graph_days: { type: 'integer', minimum: 1 },
        reports_directory: { type: 'string' },
        graphs_directory: { type: 'string' },
...
| --- | ---: | ---: | ---: |
{{#file_deltas}}
| [{{{path_code}}}]({{{url}}}) | {{base}} | {{current}} | {{delta_label}} |
{{/file_deltas}}
...
export function normalizeConfig(
  ...
  return {
    ...
    publish: {
      ...
      history_filename: config.publish?.history_filename ?? 'history.json',
      graph_days: config.publish?.graph_days ?? 30,
      reports_directory: config.publish?.reports_directory ?? 'reports',
      graphs_directory: config.publish?.graphs_directory ?? 'graphs',
